class User < ActiveRecord::Base
  ...
  validates :first_team_name, :presence => true, :length => { :maximum => 30 }, :on => :create
  ...
  after_create -> { Team.create! :name => first_team_name, :leader_id => self.id if first_team_name.present? }
